...
class TestDbIndexSqlite(TestSimpleIndex):
    ...
    def setup(self):
        ...
        self.index = dpm.index.db.DbIndexSqlite(self.dburi)
        self.index.init()
...
class TestDbIndexSqlachemy(TestSimpleIndex):
    ...
    def setup(self):
        if os.path.exists(self.tmpfile):
            os.remove(self.tmpfile)
        self.index = dpm.index.db.DbIndexSqlalchemy(self.dburi)
        self.index.init()
    ...
    def test_list(self):
        ...
        pkg = dpm.package.Package(name=u'blah', id=u'abc')
        self.index.register(pkg)
        pkgs = self.index.list()
    ...
    def test_get_when_loaded_as_new_and_init_not_called(self):
        pkg_name = u'blah'
        pkg = dpm.package.Package(name=pkg_name, id=u'abc')
        self.index.register(pkg)
